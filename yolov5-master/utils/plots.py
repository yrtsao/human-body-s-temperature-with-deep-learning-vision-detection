...
import utils.temperature as tp 
...
"""
def plot_one_box_fir(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
...
"""
def plot_one_box_fir(x, img, img_r, M, T, color=None, label=None, line_thickness=None):
...
"""
def plot_one_box_fir(x, img, img_r, M, color=None, label=None, line_thickness=None):
    ...
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, tl, lineType=cv2.LINE_AA) # %%有修改
    if label:
        tf = max(tl - 1, 1)  # font thickness
        # print("imCrop",imCrop)
        isperson = label[:6]
        # print("label:",isperson)
        if isperson == 'person':
            face_loc = tp.face_dect(img)
            # print("face_loc",face_loc)
            
            if face_loc != ():  
                # print("loc")
                (x_c,y_c,w_c,h_c) = face_loc
             
                if c1[0] <= x_c <= c2[0] and c1[0] <= x_c + w_c <= c2[0] and c1[1] <= y_c <= c2[1] and c1[1] <= y_c + h_c <= c2[1]:
                    c1_r, c2_r = tp.Ptransform([x_c,y_c], [x_c + w_c ,y_c + h_c], M)
                    imCrop = img_r[c1_r[0] : c2_r[0], c1_r[1] : c2_r[1]]
                    # print(imCrop.shape)
                    if imCrop.shape[0] and imCrop.shape[1] != 0:
                        c1_r = tuple(c1_r)
                    ...
                    c1_r, c2_r = tp.Ptransform(c1, c2, M)
                    imCrop = img_r[c1_r[1] : c2_r[1], c1_r[0] : c2_r[0]]
                    tep_color = tp.get_temperature(imCrop)
                    label = label +" , Tc = "+str(tep_color)
